<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:84:"D:\wamp64\www\game\thinkphp_5\public/../application/admin\view\admin\member_del.html";i:1508916848;}*/ ?>
﻿<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<?php foreach($userinfo as $key=>$val): ?>
			<tr class="text-c">
				...
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')"><?php echo $val['username']; ?></u></td>
				<?php if($val['sex']==1): ?>
				<td>男</td>
				<?php else: ?>
				<td>女</td>
...
function member_huanyuan(obj,id){
	layer.confirm('确认要还原吗？',function(index){
		...
		$.ajax({  
            type: "post",  
            url: "<?php echo url('admin/adminuser/huishou_guanli'); ?>",  
            data: {  
                'id':id,  
                // 'memberId':memberId  
            },  
...
function member_del(obj,id){
	layer.confirm('确认要彻底移除该用户吗？',function(index){
		$.ajax({  
            type: "post",  
            url: "<?php echo url('admin/adminuser/huishou_del'); ?>",  
            data: {  
                'id':id,  
                // 'memberId':memberId  
            },  
